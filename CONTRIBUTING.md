...
- After push and before requesting review, record the commit SHA, changed files, verification commands, known risks, and CI status as `not observed`, `pending`, `passed`, or `failed`. Pending or not-observed CI must be reported as such and must not be described as passed; completion may rely on local verification plus the final clean recheck only when CI is unavailable or still delayed and no failure is known.
