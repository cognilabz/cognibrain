...
Before non-trivial coding, debugging, CI repair, benchmark, connector, or user-preference-sensitive tasks, actively query the daemon-backed CLI lifecycle: \`node ${join(root, "bin", "cognibrain.mjs")} --runtime-root ${launchCwd} context --task "<task>" --app codex --agent codex --json\`. Do not wait for memories to appear in the prompt. Parse the returned JSON, including \`data.context\`, \`data.sections[].evidence[]\`, \`data.excludedStaleRules[]\`, \`data.id\`, and \`data.evidencePackId\`. If \`data.context\` is empty but \`data.sections[].evidence[]\` is non-empty, Cognibrain still delivered memories; use that evidence list as an automated review queue for targeted code/test verification. Use delivered context first: if the returned context or evidence pack already answers where to inspect, what command to avoid, or which prior decision matters, act from that evidence and avoid rediscovering it with another search. Before shell commands, dependency changes, migrations, or file edits with durable side effects, run \`node ${join(root, "bin", "cognibrain.mjs")} --runtime-root ${launchCwd} guard --action "<command>" --json\`. If this host exposes cognibrain MCP tools, they are optional native adapters for the same lifecycle contract.
