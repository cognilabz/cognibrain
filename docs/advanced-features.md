...
Extraction reports now include rule/provider/enrichment stages, confidence scores, failure records, enrichment candidates, and learned-rule suggestions. The deterministic rules handle text, code, and document-like content locally. Audio, image, and video envelopes carry `mediaType`, `language`, `uri`, and `mimeType` metadata and can trigger a configured JSON-command extractor for OCR, ASR, vision, or translation-backed extraction. `/ingest/media` can translate non-English media transcripts before extraction and keeps `translatedFrom` plus original content metadata for audit. Failures become suggested regex/provider/translation improvements so teams can refine extraction rules from real misses. This keeps the default install local-first while giving teams a clean provider hook for richer media and multilingual workflows.
