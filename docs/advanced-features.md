...
Built-in domain modules package coding, research, legal, finance, healthcare, security, and strict-privacy defaults with aliases, enrichment rules, retrieval profiles, lifecycle hints, and redaction policy where appropriate. Installing a domain module loads aliases into entity canonicalization, creates a `domain:<id>` retrieval profile when weights are declared, and applies enrichment rules to subsequent writes. Invalid signatures or unsupported cognibrain versions block installation. `managedMigrationBundle()` exports memories, profiles, personas, connectors, marketplace modules, policy/retention rules, compliance metadata, Docker/Compose/Kubernetes deployment artifact references, and a managed import workflow for teams that later move from local-first to managed hosting. `createManagedTenant()` stores hosted tenant metadata, and `managedControlPlaneReport()` turns tenant, autoscaling, backup, SSO, secret-manager, storage, key-provider, migration and transport status into a single publish-gate report. `importMigrationBundle()` restores the exported runtime state, and `verifyBackupRecovery()` proves encrypted memories are recoverable from the provided key material after export or key metadata rotation.

`CognibrainClient` in `src/sdk/client.ts` wraps the HTTP API for TypeScript callers, while lightweight Python, Go, and Rust client surfaces live under `sdk/`. `/sdk/openapi` returns the generated API/client description for codegen and external package publishing.

Official connector manifests are seeded for email, chat, project management, docs, code, calendars, and cloud storage. Each manifest declares direction, auth style, capabilities, default source kind, metadata mapping, privacy policy, list/poll endpoint contracts, and allowed writeback operations so integrations can be installed consistently rather than inventing per-harness payloads. `listConnectorItems()` calls external list endpoints without writing memory. `pollConnector()`, `syncConnectorEvents()`, and `/connectors/sync` convert external events into add-only extraction, store sync records, preserve external ids, enforce `never_store` privacy policy, and emit connector audit events. `writebackConnector()` and `/connectors/writeback` render source-specific export payloads for email, chat, issue trackers, docs, code review, calendars, and custom systems, then either return a queued dry-run plan or deliver it through a configured HTTP endpoint with signed headers and status-code capture. `recordConnectorFeedback()` and `/connectors/feedback` convert accepted changes, rejected suggestions, failing tests, and user corrections into trust/importance learning. `/connectors/health` exposes last status, last sync/writeback timestamps, capabilities, privacy policy, and error state, and the dashboard reads the same endpoint for operator metrics. The same API can register custom two-way connectors for writeback-capable harnesses while rejecting invalid manifests such as ingest-only writeback.
...
`storageStatus()` exposes the active adapter and the supported local-first plus production backend contracts: in-memory for tests, atomic JSON snapshots for desktop/CLI, append-only JSONL for replayable durable logs, SQLite transactional snapshots with an append-only SQL event table when `node:sqlite` is available, Postgres/Cockroach-compatible SQL-shaped snapshots, Cassandra-compatible wide-column snapshots with partition/clustering keys, psql-backed Postgres/Cockroach remote drivers, and cqlsh-backed Cassandra remote drivers. JSON-to-SQL, Cassandra-compatible migration boundaries, and remote-driver capability reporting are covered by tests; live remote clusters still need deployment-specific credentials and service integration tests.
