...
Built-in domain modules package coding, research, legal, finance, healthcare, security, and strict-privacy defaults with aliases, enrichment rules, retrieval profiles, lifecycle hints, and redaction policy where appropriate. `managedMigrationBundle()` exports memories, profiles, personas, connectors, marketplace modules, retention rules, and compliance metadata with backup, SSO, and secret-manager placeholders for teams that later move from local-first to managed hosting.
