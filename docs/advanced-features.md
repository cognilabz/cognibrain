...
Official connector manifests are seeded for email, chat, project management, docs, code, calendars, and cloud storage. Each manifest declares direction, auth style, capabilities, default source kind, metadata mapping, and allowed writeback operations so integrations can be installed consistently rather than inventing per-harness payloads. `syncConnectorEvents()` and `/connectors/sync` convert external events into add-only extraction, store sync records, preserve external ids, and emit connector audit events. `writebackConnector()` and `/connectors/writeback` render source-specific export payloads for email, chat, issue trackers, docs, code review, calendars, and custom systems, then either return a queued dry-run plan or deliver it through a configured HTTP endpoint with signed headers and status-code capture. The same API can register custom two-way connectors for writeback-capable harnesses while rejecting invalid manifests such as ingest-only writeback.
