...
curl -X POST http://localhost:8787/context/enrich \
  -H "content-type: application/json" \
  -d '{"userId":"dev","query":"Fix PR 321 and check CB-9","primaryIssueStore":"official-github","primaryKnowledgeStore":"official-confluence","defaultSearchConnectors":["official-jira"],"tokenBudget":1200}'
...
The service seeds official manifests for email, chat, project management, docs, code, calendar, cloud storage and native vendor drivers for GitHub, GitLab, Azure DevOps, Slack, Discord, Teams, Jira, Confluence, Notion, Linear, Gmail, Google Drive, Google Calendar, Asana, ClickUp, Sentry, Datadog, PagerDuty and PostHog. Custom manifests declare direction, auth, capabilities, default source kind, metadata mapping, privacy policy, list/poll endpoints, OAuth metadata, and writeback endpoint metadata. `/connectors/auth/begin` creates an OAuth authorization URL with state/scopes/redirect URI, `/connectors/auth/callback` stores only a token reference plus hash and applies that `authRef` to connector list/poll/writeback blocks, and `/connectors/auth` reports session status. `/connectors/list` calls a connector list endpoint and returns normalized external items. `/connectors/poll` calls a poll endpoint, ingests returned events through the same add-only extraction path, and records last state for `/connectors/health`. A `privacyPolicy` of `never_store` records the poll but writes no memories. `/connectors/sync` maps explicit external events into add-only extraction, records connector sync status, emits audit events, and stores `provenance.sourceRef` with `connectorId`, `externalId`, source URL, author, timestamp, version, and a stable event hash when available. `/connectors/writeback` renders source-specific export payloads for email replies, chat posts, issue updates, doc comments, code review comments, calendar notes, cloud-storage metadata, incident notes, feature flags, or generic custom connectors. With `dryRun:true` or omitted, the response is an auditable queued writeback plan. With a manifest `writeback.endpoint` and `dryRun:false`, Cognibrain sends the plan as an HTTP request with connector headers, optional HMAC signature, status-code capture, and timeout governed by `MEMORY_CONNECTOR_TIMEOUT_MS` or 10 seconds by default; native `vendor://` connectors call the source API directly and persist redacted request plans. `/connectors/feedback` maps accepted changes, rejected suggestions, failing tests, and user corrections into trust/importance feedback plus an auditable connector feedback memory. `/providers` reports whether JSON-command intelligence is active and which tasks fall back deterministically. `/translate` and `/ingest/media` support multilingual transcripts plus local OCR/ASR/video-frame metadata through `metadata.ocrText`, `metadata.asrText`, and `metadata.frames` while preserving `uri`, `mimeType`, original media content, and transformation history. `mediaType:"document"` with `application/pdf` uses the same local OCR envelope for PDF snapshots.
...
Intent reports classify queries as fact, temporal, multi-hop, procedural/preference, contradiction, project, personal, team, or connection-explanation requests. Each report also includes a concrete `plan` with query type, secondary query types, retrieval strategies, mode, confidence, weight hints, and explanation. Retrieval uses the plan to choose `hybrid` vs. `path` mode and adjust weights when the caller has not explicitly selected a profile or mode, and search/evidence responses carry the selected plan with each result.
...
Mutation and governance events are exported as a hash-linked journal:
...
`GET /audit/chain` returns the chronological audit events with canonical journal event types such as `memory.created`, `memory.updated`, `memory.archived`, `memory.deleted`, `context_pack.created`, `policy.denied`, `dream.action`, and `connector.ingested`. Each event has `sequence`, `previousHash`, `payloadHash`, and `hash`, plus a replay report that reconstructs memory lifecycle state, context-pack creation, policy denials, connector events, and dream actions. Use `memoryId`, `userId`, or `type` query parameters to narrow the export while keeping the original event hashes intact.
...
Search modes are `hybrid`, `rrf`, `graph`, and `path`. Expansion can be deterministic, provider-backed through the JSON-command intelligence adapter, or caller-supplied with `queryExpansions`. Results include explanations, `queryPlan`, calibrated `confidence`, `unsafeToInject`, `retrievalMode`, `expandedQueries`, `fusion`, graph path hints, optional contradiction metadata, and context-verification decisions when available. Evidence/context packs skip low-confidence unsafe results instead of blindly injecting every ranked hit.
...
curl -X POST http://localhost:8787/memories/mem_123/revert \
  -H "content-type: application/json" \
  -d '{}'
curl -X POST http://localhost:8787/memories/mem_123/archive
