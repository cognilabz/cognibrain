...
| CB-MCP | MCP is the preferred agent integration surface. | `npm test`, MCP server smoke | `src/connectors/mcpServer.ts`, `src/connectors/mcpHandlers.ts`, `src/connectors/mcpTools.ts` | CLI is fallback/operator path. |
...
| CB-STORAGE-BOUNDARY | DB-primary row persistence exists with snapshots retained as backup/compaction artifacts. | `npm run internal -- verify:postgres`, `npm test -- tests/core.test.ts` | `src/api/persistence/`, `src/api/repositories/` | Rerun on the target database before production claims. |
