...
| CB-CONNECTORS | Native drivers exist for the listed source systems. | `npm run verify:vendor-connectors` | `src/connectors/vendorConnectors.ts`, `artifacts/vendor-connectors-live.json` | Credential smoke requires tenant credentials and is environment-specific. |
...
| CB-ARENA | Benchmark Arena compares Cognibrain with Mem0, Graphiti/Zep, Cognee, LangMem and GBrain on the same local scenario stream. | `npm run benchmark:arena` | `artifacts/arena/run.json`, `public/benchmark-arena/results.json` | Competitor rows are API-shape adapters, not vendor-hosted certifications. |
