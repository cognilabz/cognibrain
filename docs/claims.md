...
Mem0 and LangMem are now checked through real same-run-native package runners. GBrain is checked as a real same-run-cli competitor row. Graphiti/Zep and Cognee have native runners but are credential-blocked in the checked artifact when LLM credentials are absent; no API-shape score is substituted. Current checked connector state: 19 hermetic drivers, 19 API/spec-verified drivers, 19 live-smoke-ready drivers, 0 tenant-verified live smokes and 0 production certifications.
