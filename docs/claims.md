...
| CB-TRUTH-GATE | Product readiness is audited from code and generated reports before claims are accepted. | `npm run audit:truth`, `cognibrain proof` | `scripts/release/audit-product-truth.mjs`, generated local reports | The gate passes when claims are honest; reports are internal outputs. |
...
| CB-RELEASE | The repo has a single release gate. | `npm run release:check` | `scripts/release/release-check.mjs` | Passing locally does not equal a managed SaaS SLA. |
