...
const checks = [
  ...
  check("README represents the product clearly", [
    ...
    has(files.readme, "competitor rows are local API-shape compatibility adapters unless their proof level says otherwise"),
  ...
  check("benchmark claims are bounded by artifacts", [
    ...
    has(files.benchmarks, "GBrain is now checked as a real same-run-cli competitor row"),
    has(files.benchmarks, "Mem0 remains same-run-api-shape in the checked artifact because no MEM0_API_KEY was available"),
  ...
  check("connectors, adapters and SDK are discoverable", [
    ...
    has(files.integrations, "Current checked connector state: 19 hermetic drivers, 19 API/spec-verified drivers, 0 tenant live smokes and 0 production certifications."),
  ...
  check("claim map keeps marketing honest", [
    ...
    has(files.claims, "GBrain is now checked as a real same-run-cli competitor row."),
    has(files.claims, "Mem0 remains same-run-api-shape in the checked artifact because no MEM0_API_KEY was available."),
  ...
  check("market pages are proof-first, not slogan-only", [
    ...
    has(files.sameBenchmark, "GBrain is now checked as a real same-run-cli competitor row."),
    has(files.sameBenchmark, "Mem0 remains same-run-api-shape in the checked artifact because no MEM0_API_KEY was available."),
