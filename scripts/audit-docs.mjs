...
const checks = [
  ...
  check("benchmark claims are bounded by artifacts", [
    ...
    has(files.benchmarks, "Mem0 and LangMem are now checked through real same-run-native package runners"),
  ...
  check("claim map keeps marketing honest", [
    ...
    has(files.claims, "Graphiti/Zep and Cognee are credential-blocked in the checked artifact because no LLM/vendor credentials were available."),
  ...
  check("market pages are proof-first, not slogan-only", [
    ...
    has(files.sameBenchmark, "Graphiti/Zep and Cognee are credential-blocked in the checked artifact unless LLM/vendor credentials are supplied."),
