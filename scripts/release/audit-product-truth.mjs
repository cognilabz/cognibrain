...
const files = {
  ...
  cli: readMany(["bin/cognibrain.mjs", "bin/lib/render.mjs", "bin/lib/cliRuntime.mjs", "bin/lib/harnessRuntime.mjs", "bin/lib/resourcesRuntime.mjs", "bin/lib/daemonClient.mjs", "bin/lib/lightweightMcpServer.mjs", "scripts/runtime/start-local.mjs", "scripts/runtime/build-api.mjs"]),
  ...
  serviceEngineering: read("src/api/service/engineering.ts"),
...
const compactBenchmarkStdout = [files.cognicodeBenchSource, files.arenaSource, files.nextgenBenchmarksSource].every((source) =>
  source.includes("function printCliReport") &&
  source.includes("MEMORY_FULL_BENCHMARK_STDOUT") &&
  source.includes("--json-stdout") &&
  source.includes("outputPath")
...
const streamingBenchmarkOrchestrators = [
  files.hardArenaBenchmark,
  files.nativeCompetitorBenchmark,
  files.operatorMemoryNativeCompetitorBenchmark,
  files.realworldNativeCompetitorBenchmark
].every((source) =>
  source.includes("streaming-command.mjs") &&
  source.includes("await runCommand(") &&
  !source.includes("spawnSync") &&
  !source.includes("maxBuffer")
) &&
  files.streamingCommandHelper.includes("export function runCommand") &&
...
const transientContextPackRetention = files.service.includes("pruneTransientContextPacks") &&
  files.service.includes("MEMORY_EVIDENCE_PACK_RETENTION_MAX") &&
  files.service.includes("MEMORY_CODING_CONTEXT_PACK_RETENTION_MAX") &&
  files.service.includes("pruneGeneratedMap") &&
  files.coreTests.includes("bounds transient context pack retention in runtime and local snapshots") &&
  files.coreTests.includes("compacts over-retained transient context packs during startup load");
const streamingCompetitorWrappers = [
  files.arenaMem0Runner,
  files.arenaGbrainRunner,
  files.arenaNativePythonRunnerWrapper,
  ...
  files.arenaMem0Runner.includes("await runCommand(\"npm\"") &&
  files.arenaGbrainRunner.includes("await runGBrain(") &&
  files.arenaGbrainRunner.includes("return runCommand(\"bun\"") &&
  files.arenaNativePythonRunnerWrapper.includes("!result.truncatedStdout") &&
  files.operatorMemoryNativePythonRunnerWrapper.includes("!result.truncatedStdout") &&
  files.arenaNativePythonRunnerWrapper.includes("stdout exceeded bounded output capture") &&
  files.operatorMemoryNativePythonRunnerWrapper.includes("stdout exceeded bounded output capture");
const streamingArenaParentCommands = files.arenaSource.includes("function runBoundedArenaCommand") &&
  files.arenaSource.includes("createBoundedTextCapture") &&
  files.arenaSource.includes("truncatedStdout") &&
  files.arenaSource.includes("MEMORY_ARENA_RUNNER_OUTPUT_LIMIT") &&
  files.arenaSource.includes("MEMORY_ARENA_JUDGE_OUTPUT_LIMIT") &&
  files.arenaSource.includes("MEMORY_ARENA_QUALITY_JUDGE_OUTPUT_LIMIT") &&
  files.arenaSource.includes("runner stdout exceeded bounded capture") &&
  files.arenaSource.includes("Arena quality judge stdout exceeded bounded capture") &&
  files.arenaSource.includes("await runBoundedArenaCommand(this.runner.command") &&
  files.arenaSource.includes("await runBoundedArenaCommand(command, []") &&
  !files.arenaSource.includes("spawnSync") &&
  !files.arenaSource.includes("maxBuffer");
const reflectionSinglePassSnapshots = files.reflectionSource.includes("const activeForSummaries = this.activeMemories(userId)") &&
  files.reflectionSource.includes("const activeWithReflections = created.length ? this.activeMemories(userId) : activeForSummaries") &&
  files.reflectionSource.includes("this.summarizeClusters(userId, activeForSummaries, now)") &&
  files.reflectionSource.includes("this.extractBehavioralPatterns(userId, activeForSummaries, now, evaluations)") &&
  files.coreTests.includes("runs reflection with bounded active-memory store scans") &&
  files.coreTests.includes("expect(store.listCalls).toBeLessThanOrEqual(3)");
const autoDreamMaintenanceIndexed = files.service.includes("for (const [userId, status] of Object.entries(this.maintenance.users))") &&
  files.service.includes("if (status.writesSinceDream <= 0) continue") &&
...
const dreamPlanNotDueFastPath = files.dreamRuntime.includes("canSkipMemoryScan") &&
  files.dreamRuntime.includes("trigger === \"harness_session_end\" || trigger === \"auto_interval\"") &&
  files.dreamRuntime.includes("memoryScanSkipped: true") &&
  files.dreamRuntime.includes("quick dream plan is not due by maintenance counters") &&
  files.coreTests.includes("plans not-due harness session-end dreams without scanning the memory store") &&
  files.coreTests.includes("expect(listCalls).toBe(0)");
...
const runtimeHarnessOnlyProviderActivation = files.serviceEngineering.includes("export function providerFromEnv(): NonNullable<MemoryServiceOptions[\"intelligence\"]>") &&
  files.serviceEngineering.includes("return {};") &&
  !files.service.includes("command: process.env.MEMORY_INTELLIGENCE_COMMAND") &&
  files.coreTests.includes("keeps runtime intelligence provider activation explicit to the harness") &&
  files.coreTests.includes("providerStatus().command).toBeUndefined");
const runtimeLazyPostgresDriver = files.service.includes('await import("../repositories/postgresRepository")') &&
  files.service.includes('backend === "postgres-repository"') &&
  !files.service.includes("new PostgresMemoryRepository(process.env.MEMORY_POSTGRES_URL)") &&
  !files.service.includes("import { AsyncPostgresMemoryRepository") &&
  !files.service.includes("import { PostgresMemoryRepository") &&
  files.cli.includes('external: ["pg"]') &&
  files.cli.includes("splitting: true") &&
  files.cli.includes('chunkNames: "chunks/[name]-[hash]"') &&
  files.coreTests.includes("keeps the Postgres repository driver-backed with service-state tables and optional RLS") &&
  files.coreTests.includes('external: ["pg"]') &&
  files.coreTests.includes("splitting: true");
...
const benchmarkSvgClaimBoundary = files.benchmarkSvgSource.includes("publicClaimAllowed") && files.benchmarkSvgSource.includes("publicClaimDetail") && files.benchmarkSvgSource.includes("boundary-missing") && files.benchmarkSvgSource.includes("arenaProofDetail") && files.benchmarkSvgSource.includes("Synthetic scenario diagnostics only") && files.benchmarkSvgSource.includes("Each bar is diagnostic only with claim blocked") && files.benchmarkSvgSource.includes("Docs-visible percent charts") && files.benchmarkSvgSource.includes("not quality, competitor, or market-leadership proof") && !files.benchmarkSvgSource.includes('row.label === "Cognibrain full"') && files.benchmarkSvg.includes("claim blocked") && files.benchmarkSvg.includes("diagnostic pass") && files.benchmarkSvg.includes("api-shape diagnostic") && files.benchmarkSvg.includes("not market proof") && files.benchmarkSvg.includes("diagnostic only") && files.benchmarkSvg.includes("ablation diagnostic") && files.benchmarkSvg.includes("internal diagnostic");
