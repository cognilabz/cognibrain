...
const checks = [
  ...
  check("storage.db-primary-event-proof", "Storage plan has row-backed SQLite/Postgres paths, append-only audit/event proof and no open plan gap artifact.", [
    ...
    files.memoryRoutes.includes("await defaultService.recordHarnessActionAsync") && files.memoryRoutes.includes("await defaultService.recordCodeCorrectionAsync") && files.engineeringRuntime.includes("service.addAsync(codeCorrectionMemoryInput") && files.engineeringRuntime.includes("applySupersessionAsync") && files.engineeringRuntime.includes("derivedCorrectionInputs") && files.engineeringRuntime.includes("service.updateAsync(memory.id") && files.coreTests.includes("commits production code corrections and derived memories through async UnitOfWork writes before returning") && files.memoryRoutes.includes("await defaultService.updateAsync") && files.memoryRoutes.includes("archivedAt: z.string().optional()") && files.apiTests.includes("archivedAt =") && files.apiTests.includes("expect(patched.archivedAt).toBe(archivedAt)") && files.memoryRoutes.includes("await defaultService.archiveAsync") && files.memoryRoutes.includes("await defaultService.deleteAsync"),
