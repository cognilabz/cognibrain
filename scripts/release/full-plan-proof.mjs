...
const files = {
  ...
  apiServer: read("src/api/server.ts"),
  ...
  engineeringRuntime: read("src/api/service/engineeringRuntime.ts"),
...
const checks = [
  ...
  check("storage.db-primary-event-proof", "Storage plan has row-backed SQLite/Postgres paths, append-only audit/event proof and no open plan gap artifact.", [
    ...
    files.memoryRoutes.includes("await defaultService.extractAsync") && files.memoryRoutes.includes("await defaultService.ingestMediaAsync") && files.memoryServiceStore.includes("extractAsync(") && files.memoryServiceStore.includes("ingestMediaAsync(") && files.memoryServiceStore.includes("createEpisodeAsync(input: EpisodeInput): Promise<EpisodeRecord>") && files.storeRuntime.includes("export async function extractAsync") && files.storeRuntime.includes("service.createEpisodeAsync") && files.eventJournal.includes("episode.created") && files.eventJournal.includes("episode.updated") && files.storeRuntime.includes("type: \"episode.created\"") && files.storeRuntime.includes("type: \"episode.updated\"") && files.storeRuntime.includes("service.addAsync(linkStateChange") && files.storeRuntime.includes("applySupersessionForExtractionAsync") && files.coreTests.includes("commits production extract and media ingest memories through async UnitOfWork writes") && files.coreTests.includes("extractAsync must not use sync createEpisode") && files.coreTests.includes("does not publish extraction episodes or memories when the production episode event fails") && files.memoryRoutes.includes("await defaultService.recordHarnessActionAsync") && files.memoryRoutes.includes("await defaultService.recordCodeCorrectionAsync") && files.engineeringRuntime.includes("recordCodeCorrectionInUnitOfWork") && files.engineeringRuntime.includes("executeUnitOfWork(async (uow)") && files.engineeringRuntime.includes("uow.memoryRepository.create") && files.engineeringRuntime.includes("uow.memoryRepository.update") && files.engineeringRuntime.includes("uow.claimRepository.register") && files.engineeringRuntime.includes("uow.truthRepository.decide") && files.engineeringRuntime.includes("atomicUnitOfWork: true") && files.engineeringRuntime.includes("postCommitWriteTriggers") && files.engineeringRuntime.includes("triggerPostCommitWrites") && files.coreTests.includes("commits production code correction pipeline in one async UnitOfWork") && files.coreTests.includes("postCommitWrites") && files.coreTests.includes("writesSinceDream") && files.coreTests.includes("rolls back production code correction pipeline read models when UnitOfWork truth writes fail") && files.memoryRoutes.includes("await defaultService.patchEvidenceTrailAsync") && files.engineeringRuntime.includes("patchEvidenceTrailAsync") && files.engineeringRuntime.includes("patch_evidence.created") && files.engineeringRuntime.includes("productionUnitOfWork: true") && files.coreTests.includes("commits production patch evidence through a UnitOfWork event before returning") && files.coreTests.includes("does not publish patch evidence read models when UnitOfWork event append fails") && files.memoryRoutes.includes("await defaultService.updateAsync") && files.memoryRoutes.includes("await defaultService.updateConsentAsync") && files.memoryRoutes.includes("await defaultService.revertMemoryAsync") && files.dreamRoutes.includes("await defaultService.confirmMemoryAsync") && files.dreamRoutes.includes("await defaultService.retractMemoryAsync") && files.coreTests.includes("commits production lifecycle memory updates through async UnitOfWork and post-commit write triggers") && files.coreTests.includes("postCommitWrites") && files.memoryRoutes.includes("archivedAt: z.string().optional()") && files.apiTests.includes("archivedAt =") && files.apiTests.includes("expect(patched.archivedAt).toBe(archivedAt)") && files.memoryRoutes.includes("await defaultService.archiveAsync") && files.memoryRoutes.includes("await defaultService.deleteAsync"),
  ...
  check("dream.durable-queue-proof", "Dream jobs are persisted, controllable and release/source-aware across service, HTTP, MCP and tests.", [
    ...
    files.dreamWorkerRuntime.includes("productionDreamWorkerMode"),
    files.dreamWorkerRuntime.includes("in-process fallback is disabled"),
    files.dreamRuntime.includes("Production dream job execution must be claimed"),
    ...
    files.apiServer.includes("productionMode()") && files.apiServer.includes("defaultService.runDueDreamJobsAsync()") && files.apiServer.includes("defaultService.runDueDreams()"),
