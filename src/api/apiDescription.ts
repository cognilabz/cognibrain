import { apiOperationContract, apiRouteMethods } from "./releaseContract";
...
export function buildApiDescription(auth?: ApiDescriptionAuth) {
  ...
  const routeMethods = apiRouteMethods();
  return {
    openapi: "3.1.0",
...
function openApiOperation(path: string, method: string, security: Array<Record<string, string[]>>): Record<string, unknown> {
  const contract = apiOperationContract(path, method);
  ...
  return {
    operationId,
    summary: `${method} ${path}`,
    "x-cognibrain-stability": contract?.stability ?? "uncontracted",
    "x-cognibrain-surface": contract?.surface ?? "uncontracted",
