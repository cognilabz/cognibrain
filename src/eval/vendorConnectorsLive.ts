...
function routeVendorFixture(url: URL, method: string, body: string, response: ServerResponse): void {
  ...
  if (method === "POST" && url.pathname === "/linear/graphql") {
    ...
    return send(response, 200, { data: { issues: { nodes: [{ id: "lin_123", identifier: "CB-42", title: "Correction: do not skip connector compatibility proof", url: "https://linear.app/cognibrain/issue/CB-42", updatedAt: "2026-05-25T07:30:00.000Z", state: { name: "In Progress" }, assignee: { name: "Maintainer" }, labels: { nodes: [{ name: "plan1_5" }, { name: "connector" }] }, comments: { nodes: [{ id: "lc1", body: "Use hermetic vendor fixtures instead and rerun the live verifier.", user: { name: "Reviewer" }, updatedAt: "2026-05-25T07:31:00.000Z" }] } }] } } });
  }
