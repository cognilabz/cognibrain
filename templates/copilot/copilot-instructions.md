...
- Use `__COGNIBRAIN_COMMAND__ start` to start the local API, `__COGNIBRAIN_COMMAND__ status` to inspect it, and `__COGNIBRAIN_COMMAND__ stop` to stop it. Add `--dashboard` only when a browser dashboard is needed.
...
- Use the CLI lifecycle as the default agent path:
  - `__COGNIBRAIN_COMMAND__ context --task "<task>" --app copilot --agent copilot --json` before non-trivial work,
  - `__COGNIBRAIN_COMMAND__ guard --action "<command>" --json` before shell commands or file edits with durable side effects,
  - `__COGNIBRAIN_COMMAND__ outcome --command "<command>" --exit-code <code> --json` after important tool results,
  - `__COGNIBRAIN_COMMAND__ patch-evidence --task "<task>" --json` after non-trivial patches.
