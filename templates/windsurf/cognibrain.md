...
- Actively query project memory with `__COGNIBRAIN_COMMAND__ context --task "<task>" --app windsurf --agent windsurf --json` before changing files with repo-specific rules; do not wait for memories to appear in the prompt.
...
- Respect `__COGNIBRAIN_COMMAND__ guard --action "<command>" --json` warnings for forbidden commands and generated files.
